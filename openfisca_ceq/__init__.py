...
def add_variables_from_file(
    ...
    try:
        ...
        try:
            ...
            raise
        potential_variables = [
            getattr(module, item) for item in dir(module) if not item.startswith("__")
        ]
        for pot_variable in potential_variables:
            ...
            if (
                ...
                if (
                    ignored_variables is not None
                    and pot_variable.__name__ in ignored_variables
                ):
                    continue
                elif (
                    replaced_variables is not None
                    and pot_variable.__name__ in replaced_variables
                ):
                    self.replace_variable(pot_variable)
                elif (
                    updated_variables is not None
                    and pot_variable.__name__ in updated_variables
                ):
...
def add_variables_from_directory(
    self,
    directory,
    ignored_variables=None,
    replaced_variables=None,
    ...
    for py_file in py_files:
        self.add_variables_from_file(
            py_file, ignored_variables, replaced_variables, updated_variables
        )
    subdirectories = glob.glob(path.join(directory, "*/"))
    for subdirectory in subdirectories:
        self.add_variables_from_directory(
            subdirectory, ignored_variables, replaced_variables, updated_variables
        )
