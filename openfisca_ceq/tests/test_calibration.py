...
def test_calibration(country):
    ...
    log.info("VA brut: {}".format(gross_value_added_target / 1e9))
    log.info(
        "Salaire brut before calibration: {}".format(
            survey_scenario.compute_aggregate("salaire_brut", period=year) / 1e9
        )
    )
    log.info(
        "Salaire super brut before calibration: {}".format(
    ...
    share_by_variable, target_by_variable = (
        survey_scenario.inflate_variables_sum_to_target(
    ...
    log.info(
        "Salaire brut after calibration: {}".format(
            survey_scenario.compute_aggregate("salaire_brut", period=year) / 1e9
        )
    )
    log.info(
        "Salaire super brut after calibration: {}".format(
            survey_scenario.compute_aggregate("salaire_super_brut", period=year) / 1e9
        )
    )
