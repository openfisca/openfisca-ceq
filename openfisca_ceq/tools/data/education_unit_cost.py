...
def build_unit_cost_by_category_by_country():
    df = pd.read_csv(
        os.path.join(
            assets_directory, "unit_cost_of_education_public_service_by_country.csv"
        )
    )
    df.columns = [slugify.slugify(column, separator="_") for column in df.columns]
    df["country"] = df.country.apply(lambda x: slugify.slugify(x, separator="_"))
    ...
    for country in df["country"].unique():
        ...
        unit_cost_by_category = dict(
            zip(
                country_df.harmonized_category,
                country_df.unit_cost_thousands_fcfa * 1000
                if country != "mali"
                else country_df.adjusted_unit_cost_thousands_fcfa * 1000,
            )
        )
        ...
        if "secondary_and_higher_education" in unit_cost_by_category:
            unit_cost_by_category["secondary"] = unit_cost_by_category["higher"] = (
                unit_cost_by_category["secondary_and_higher_education"]
            )
            del unit_cost_by_category["secondary_and_higher_education"]

        if "basic_education" in unit_cost_by_category:
            unit_cost_by_category["primary"] = unit_cost_by_category.pop(
                "basic_education"
            )
        ...
        for variable_name, harmonized_key in harmonized_key_by_variable_name.items():
            ...
            if harmonized_key in unit_cost_by_category:
                unit_cost_by_category[variable_name] = unit_cost_by_category.pop(
                    harmonized_key
                )
