...
def build_consumption_items_list(country):
    consumption_items = build_label_by_code_coicop(
        country, additional_variables=["prod_id"]
    )
    try:
        consumption_items.prod_id = (
            consumption_items.prod_id.astype(float).astype(int).astype(str)
        )
    except Exception:
...
def load_expenditures(country):
    ...
    assert country_expenditures_variables <= set(
    ...
    if missing_products_in_legislation:
        log.info(
            "Missing product in legislation: \n {}".format(
                (
                    expenditures.reset_index()
                    .query("prod_id in @missing_products_in_legislation")
                    .filter(items=["label", "prod_id"])
                    .drop_duplicates()
                )
            )
        )

    missing_products_in_expenditures = set(
        consumption_items.prod_id.unique()
    ).difference(set(expenditures.prod_id.unique()))
    if missing_products_in_expenditures:
        log.info(
            "Missing product in expenditures: \n {}".format(
                (
                    expenditures.reset_index()
    ...
    if missing_products_in_legislation:
        expenditures = expenditures.query(
            "prod_id not in @missing_products_in_legislation"
        ).copy()

    consumption_items["poste_coicop"] = (
        "poste_"
        + consumption_items.deduplicated_code_coicop.apply(
            lambda x: slugify.slugify(x, separator="_")
        )
    )
    household_expenditures = expenditures.merge(
        consumption_items.reset_index()[["poste_coicop", "prod_id"]],
        on="prod_id",
        how="left",
    )
    household_expenditures = (
        household_expenditures.filter(["hh_id", "poste_coicop", "depense"], axis=1)
    ...
    irregular_columns = [
        column
        for column in household_expenditures.columns
        if (not str(column).startswith("poste_")) and (column != "hh_id")
    ]
    if irregular_columns:
        log.info(
            "Irregular colums in household expenditures: {}".format(irregular_columns)
        )
