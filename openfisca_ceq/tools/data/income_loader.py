...
def build_income_dataframes(country):
    ...
    for variable in income.columns:
        if variable.startswith("rev"):
            assert (
                income[variable].notnull().any()
            ), "{} income variable for {} is all null".format(variable, country)

    assert set(model_variable_by_person_variable.keys()).difference(
        set(missing_revenus_by_country.get(country, []))
    ) <= set(income.columns), "Missing {} in {} income data source".format(
        set(model_variable_by_person_variable.keys())
        .difference(set(missing_revenus_by_country.get(country, [])))
        .difference(set(income.columns)),
        country,
    )

    data_by_model_id_variable = {v: k for k, v in model_by_data_id_variable.items()}
    data_by_model_role_index_variable = {
        v: k for k, v in model_by_data_role_index_variable.items()
    }
    ...
    for entity, variables in variables_by_entity.items():
        ...
        filtered_variables = list(
            set(variables).difference(set(missing_revenus_by_country.get(country, [])))
        )
        ...
        if entity == "person":
            for group_entity in variables_by_entity.keys():
                ...
                data_group_entity_role_index += [
                    data_by_model_role_index_variable[
                        "{}_role_index".format(group_entity)
                    ]
                ]
        ...
        if entity != "person":
            ...
            household_weight = dataframe.groupby(group_id_variable)[
                person_weight_variable
            ].mean()

            weight_by_group_ok = (
                dataframe.groupby(group_id_variable)[person_weight_variable].nunique()
                == 1
            )
            problematic_group_id = (
                weight_by_group_ok.reset_index()
                .query("~{}".format(person_weight_variable))[group_id_variable]
                .tolist()
            )
    ...
    log.info(
        "For country {}: {} persons and {} households".format(
            country,
            len(dataframe_by_entity["person"]),
            len(dataframe_by_entity["household"]),
        )
    )
    assert (
        len(dataframe_by_entity["person"])
        == dataframe_by_entity["person"].pers_id.nunique()
    )
    assert (
        len(dataframe_by_entity["household"])
        == dataframe_by_entity["person"].hh_id.nunique()
    )
