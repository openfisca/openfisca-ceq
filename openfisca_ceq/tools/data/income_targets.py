...
def read_target(country, variable):
    ...
    target_variable = (
        GROSS_VALUE_ADDED if variable == "gross_value_added" else HOUSEHOLD_CONSUMPTION
    )
    ...
    target = (
        pd.read_excel(targets_file, sheet_name="calage")
        .query("iso == @code_country")
        .query("year == @year")
        .query("DESC_3 == @target_variable")["montant"]
        .copy()
    )
    ...
    log.info(
        "{} target for {} is {} Billion CFA".format(variable, country, target / 1e9)
    )
    return target
