...
def build_simulated_results(survey_scenario, index, add_country_details=False):
    simulated_amounts = pd.Series(index=index, dtype=float)
    ...
    if add_country_details:
        for variable in detailed_taxes_by_country[survey_scenario.legislation_country]:
            simulated_amounts[variable] = (
                survey_scenario.compute_aggregate(variable, period=survey_scenario.year)
                / 1e9
            )

        simulated_amounts["total_ht_imports"] = (
            compute_total_ht_imports(survey_scenario) / 1e9
        )
...
def read_tax_target(country=None, add_country_details=False):
    ...
    for country_, year in year_by_country.items():
        if country and country != country_:
            continue
        survey_scenario = build_ceq_survey_scenario(
            legislation_country=country_, year=year
        )
        inflated_survey_scenario = build_ceq_survey_scenario(
            legislation_country=country_, year=year, inflate=True
        )
        result = build_country_result(
            survey_scenario, inflated_survey_scenario, add_country_details
        )
...
def build_country_result(
    survey_scenario, inflated_survey_scenario, add_country_details=False
):
    ...
    simulated_amounts = build_simulated_results(
        survey_scenario, result.index, add_country_details
    )
    result = result.reindex(simulated_amounts.index)
    result[country_label, "direct"] = simulated_amounts

    simulated_amounts = build_simulated_results(
        inflated_survey_scenario, result.index, add_country_details
    )
