...
assert initial_revenues_source == (
    set(ceq_input_by_harmonized_variable.keys())
    .union(set(ceq_intermediate_by_harmonized_variable.keys()))
    .union(set(non_ceq_input_by_harmonized_variable.keys()))
), initial_revenues_source.difference(
    set(ceq_input_by_harmonized_variable.keys())
    .union(set(ceq_intermediate_by_harmonized_variable.keys()))
    .union(set(non_ceq_input_by_harmonized_variable.keys()))
)
...
person_variables = list(
    initial_revenues_source.union(
        set(other_model_by_harmonized_person_variable.keys())
...
class all_income_excluding_transfers(Variable):
    ...
    def formula(household, period):
        ...
        return household.sum(
            sum(household.members(variable, period) for variable in income_variables)
        )
...
class employer_other_contributions(Variable):
    def formula(household, period):
        return household.sum(household.members("famille", period)) + household.sum(
            household.members("accidents_du_travail", period)
        )
...
class social_security_contributions(Variable):
    ...
    def formula(household, period):
        ...
        return sum(household(variable, period) for variable in contribution_variables)
