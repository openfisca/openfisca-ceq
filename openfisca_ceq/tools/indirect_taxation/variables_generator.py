...
def generate_total_consumption(
    tax_benefit_system, item_variables, consumption_variable_name="consumption"
):
    def formula(household, period):
        return sum(household(variable, period) for variable in item_variables)
...
def generate_depenses_ht_postes_variables(
    tax_benefit_system, tax_name, tax_rate_by_code_coicop, null_rates=[]
):
    ...
    for tax_rate in reference_rates:
        ...
        if time_varying_rates:
            ...
            stop_years = reference_rates.start.fillna(
                GLOBAL_YEAR_STOP
            ).unique()  # last year
        ...
        for year_start, year_stop in year_stop_by_year_start.items():
            ...
            if time_varying_rates:
                filter_expression += (
                    "and (start <= @year_start) and (stop >= @year_stop)"
                )
            postes_coicop = sorted(
                tax_rate_by_code_coicop.query(filter_expression)["code_coicop"].astype(
                    str
                )
            )

            log.debug(
                "Creating fiscal category {} - {} (starting in {} and ending in {}) pre-tax expenses for the following products {}".format(
                    tax_name, tax_rate, year_start, year_stop, postes_coicop
                )
            )

            for poste_coicop in postes_coicop:
                selection = tax_rate_by_code_coicop.query(filter_expression).query(
                    "code_coicop == @poste_coicop"
                )
                ...
                functions_by_name_by_poste[poste_coicop + "_ht_hd_sd"][
                    dated_function_name
                ] = dated_func_ht_hd_sd
                functions_by_name_by_poste[poste_coicop + "_ht_hd"][
                    dated_function_name
                ] = dated_func_ht_hd
                functions_by_name_by_poste[poste_coicop + "_ht"][
                    dated_function_name
                ] = dated_func_ht
    ...
    for poste, functions_by_name in list(functions_by_name_by_poste.items()):
        if poste.endswith("_ht_hd_sd"):
            ...
            definitions_by_name = dict(
                ...
                value_type=float,
                entity=tax_benefit_system.entities_by_singular()["household"],
                label="Dépenses hors taxe et hors douane du poste_{} soumis à douane".format(
                    poste
                ),
            )
...
def depenses_ht_hd_postes_function_creator(
    ...
    assert imported_share is None or isinstance(
    ...
    def func(
        ...
        impots_indirects = parameters(
            period_arg.start
        ).prelevements_obligatoires.impots_indirects
        ...
        depenses_ttc = entity(
            "poste_" + slugify(poste_coicop, separator="_"), period_arg
        )
...
def depenses_ht_postes_function_creator(
    ...
    def func(
        entity, period_arg, parameters, tariff=tariff, imported_share=imported_share
    ):
        depenses_ht_hd = entity(
            "depenses_ht_hd_poste_" + slugify(poste_coicop, separator="_"), period_arg
        )
...
def depenses_ht_categorie_function_creator(postes_coicop, year_start=None):
    if len(postes_coicop) != 0:
        ...
        def func(entity, period_arg):
            return sum(
                entity("depenses_ht_poste_" + slugify(poste, separator="_"), period_arg)
...
def depenses_ht_hd_sd_categorie_function_creator(postes_coicop, year_start=None):
    if len(postes_coicop) != 0:
        ...
        def func(entity, period_arg):
            return sum(
                entity(
                    "depenses_ht_hd_sd_poste_" + slugify(poste, separator="_"),
                    period_arg,
                )
                for poste in postes_coicop
            )
...
def generate_fiscal_base_variables(
    tax_benefit_system, tax_name, tax_rate_by_code_coicop, null_rates=[]
):
    reference_rates = sorted(tax_rate_by_code_coicop[tax_name].unique())
    time_varying_rates = "start" in tax_rate_by_code_coicop.columns
    for tax_rate in reference_rates:
        ...
        for year_start, year_stop in year_stop_by_year_start.items():
            ...
            if time_varying_rates:
                filter_expression += (
                    "and (start <= @yyear_start) and (stop >= @yyear_stop)"
                )
            postes_coicop = sorted(
                tax_rate_by_code_coicop.query(filter_expression)["code_coicop"].astype(
                    str
            ...
            log.debug(
                "Creating fiscal category {} - {} (starting in {} and ending in {}) aggregate expenses with the following products {}".format(
                    tax_name, tax_rate, year_start, year_stop, postes_coicop
                )
            )
...
def generate_tariff_base_variables(
    tax_benefit_system, tariff_name, tax_rate_by_code_coicop, null_rates=[]
):
    ...
    for tariff_rate in reference_rates:
        ...
        for year_start, year_stop in year_stop_by_year_start.items():
            ...
            log.debug(
                "Creating tariff category {} - {} (starting in {} and ending in {}) aggregate expenses with the following products {}".format(
        ...
        definitions_by_name = dict(
            value_type=float,
            entity=tax_benefit_system.entities_by_singular()["household"],
            label="Dépenses hors taxes hors douane {} - {}".format(
...
def create_tax_formula(tax_name, tax_rate):
    def func(entity, period_arg, parameters):
        pre_tax_expenses = entity(
            "depenses_ht_{}_{}".format(tax_name, tax_rate), period_arg
        )
        rate = parameters(period_arg).prelevements_obligatoires.impots_indirects.tva[
            tax_rate
        ]
...
def create_tariff_formula(tariff_name, tariff_rate):
    def func(entity, period_arg, parameters):
        pre_tax_expenses = entity(
            "depenses_ht_hd_sd_{}_{}".format(tariff_name, tariff_rate), period_arg
        )
        rate = parameters(period_arg).prelevements_obligatoires.impots_indirects[
            tariff_name
        ][tariff_rate]
        return pre_tax_expenses * rate
...
def generate_ad_valorem_tax_variables(
    tax_benefit_system, tax_name, tax_rate_by_code_coicop, null_rates=[]
):
    reference_rates = sorted(tax_rate_by_code_coicop[tax_name].unique())
    ...
    def ad_valorem_tax_total_func(entity, period_arg):
        return sum(
            entity(class_name, period_arg) for class_name in ad_valorem_tax_components
        )

    ad_valorem_tax_total_func.__name__ = "formula_{year_start}".format(
        year_start=GLOBAL_YEAR_START
    )
    definitions_by_name = dict(
        value_type=float,
        entity=tax_benefit_system.entities_by_singular()["household"],
    ...
    tax_benefit_system.add_variable(type(class_name, (Variable,), definitions_by_name))


def generate_tariff_variables(
    tax_benefit_system, tariff_name, tax_rate_by_code_coicop, null_rates=[]
):
    reference_rates = sorted(tax_rate_by_code_coicop[tariff_name].unique())
    ...
    for tariff_rate in reference_rates:
        ...
        definitions_by_name.update(
            {"formula": create_tariff_formula(tariff_name, tariff_rate)}
        )
        tax_benefit_system.add_variable(
            type(class_name, (Variable,), definitions_by_name)
        )
    ...
    def tariff_total_func(entity, period_arg):
        return sum(entity(class_name, period_arg) for class_name in tariff_components)

    tariff_total_func.__name__ = "formula_{year_start}".format(
        year_start=GLOBAL_YEAR_START
    )
    definitions_by_name = dict(
        value_type=float,
    ...
    definitions_by_name.update({tariff_total_func.__name__: tariff_total_func})
