...
def inequality_table(
    ...
    nb_persons = (
        survey_scenario.calculate_variable(
            "number_of_people_per_household", period=period
        )
        if per_capita
        else 1
    )
...
def incidence_table(
    ...
    if not isinstance(tax_variables, str) and len(tax_variables) >= 2:
        df = pd.concat(
            [
                incidence_table(
                    survey_scenario, income_variable, [tax_variable], by_variable
                )
    ...
    series = (
        ...
        .eval(
            "{tax_variable}_incidence = {tax_variable} / {income_variable}".format(
                tax_variable=tax_variable, income_variable=income_variable
            )
        )
        .round(digits)["{}_incidence".format(tax_variable)]
    )
...
def concentration_share(
    ...
    masses = survey_scenario.compute_pivot_table(
        aggfunc="sum",
        values=[tax_variable],
        index=by_variable,
        period=survey_scenario.year,
...
def taxpayers_share(
    ...
    series = (
        ...
        .groupby(by_variable)
        .apply(lambda x: np.average(x.taxpayers, weights=x[weight_variable]))
    ).round(digits)
...
def net_payers_beneficiaries(
    ...
    tax_benefits_variables = [
        "direct_taxes",
        "indirect_taxes",
        "education_net_transfers",
    ]
    entity_key = survey_scenario.tax_benefit_system.variables[
        tax_benefits_variables[0]
    ].entity.key
    weight_variable = survey_scenario.weight_variable_by_entity[entity_key]
    groups = (
        (
...
def net_payers_beneficiaries_decile(
    ...
    series = (
        ...
        .round(digits)[["net_contribution", "net_payer"]]
    )
