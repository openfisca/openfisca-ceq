...
from openfisca_cote_d_ivoire import (
    CountryTaxBenefitSystem as CoteDIvoireTaxBenefitSystem,
)
...
class CEQSurveyScenario(AbstractSurveyScenario):
    ...
    def __init__(
        ...
        if "input_data_frame_by_entity_by_period" in data:
            ...
            for entity_dataframe in data["input_data_frame_by_entity_by_period"][
                period
            ].values():
    ...
    def inflate_to_match_gross_valued_added_and_consumption(self):
        ...
        target_variable_by_input_variable = {"salaire_brut": "salaire_super_brut"}
    ...
    def inflate_variables_sum_to_target(
        self, target_variables, target, period, target_variable_by_input_variable=dict()
    ):
        ...
        for _variable, share in share_by_variable.items():
            assert not np.isnan(share), "NaN ({}) share for variable {}".format(
                share, _variable
            )
        ...
        for (
            input_variable,
            target_variable,
        ) in target_variable_by_input_variable.items():
            self.reach_target(
                target_variable,
    ...
    def reach_target(
        self, variable, target, input_variable, period, inflator_first_guess=1.1
    ):
        initial_input_array = self.simulation.calculate_add(
            input_variable, period=period
        )
        ...
        def compute_error(inflator):
            ...
            self.simulation.set_input(
                input_variable, period, input_array
        ...
        log.info(
            "Inflate {} by {} to have aggregate of {} == {}".format(
...
def build_ceq_data(country, year=None):
    ...
    if households_missing_in_income:
        log.debug(
            "Households missing in income: \n {}".format(households_missing_in_income)
        )
    households_missing_in_expenditures = set(household_income.hh_id).difference(
        set(household_expenditures.hh_id)
    )
    if households_missing_in_expenditures:
        log.debug(
            "Households missing in expenditures: \n {}".format(
                households_missing_in_expenditures
            )
        )
    ...
    log.info(
        "{}: keeping only {} households from income data but {} are present in expenditures data".format(
            country,
            len(household_income.hh_id.unique()),
            len(household_expenditures.hh_id.unique()),
        )
    )
    ...
    if country == "mali":
        ...
        one_personne_de_reference = (
            person.query("household_role_index == 0")
            .groupby("hh_id")["household_role_index"]
            .count()
            == 1
        )

        problematic_hh_id = one_personne_de_reference.loc[
            ~one_personne_de_reference
        ].index.tolist()
        assert one_personne_de_reference.all(), "Problem with households: {}".format(
            person.loc[
                person.hh_id.isin(problematic_hh_id), ["household_role_index", "hh_id"]
            ]
        )

        if country == "mali":
            hh_id_with_missing_personne_de_reference = set(household.hh_id).difference(
                set(person.loc[person.household_role_index == 0, "hh_id"].unique())
            )
            person = person.loc[
                ~person.hh_id.isin(hh_id_with_missing_personne_de_reference)
            ].copy()
            household = household.loc[
                ~household.hh_id.isin(hh_id_with_missing_personne_de_reference)
            ].copy()
    ...
    assert (person.household_role_index == 0).sum() == len(
        person.hh_id.unique()
    ...
    model_by_data_weight_variable = {
        v: k for k, v in data_by_model_weight_variable.items()
    }
    ...
    assert person.cadre.dtype == pd.CategoricalDtype(
        categories=["non-cadre", "cadre"], ordered=True
    )
    person.cadre = person.cadre == "cadre"

    assert person.categorie_cgu.dtype == pd.CategoricalDtype(
        categories=["CGU comm/prod A", "CGU comm/prod B", "CGU service"], ordered=True
    )
    ...
    if country == "mali":  # Variable absente pour le Mali
        ...
        assert person.eleve_enseignement_public.dtype == pd.CategoricalDtype(
            categories=["public", "prive"], ordered=True
        )
    ...
    assert person.secteur_formel.dtype == pd.CategoricalDtype(
        categories=["formel", "informel"], ordered=True
    )
    ...
    input_data_frame_by_entity_by_period = {
        periods.period(year): input_data_frame_by_entity
    }
    data = dict(
...
def build_ceq_survey_scenario(
    legislation_country,
    year=None,
    data_country=None,
    inflate=False,
    adjust_indirect_taxation=False,
):
        ...
        class consumable_income(Variable):
            ...
            def formula(household, period):
                ...
                return where(
                    consumption > 0,
                    disposable_income
                    * (1 - (indirect_taxes - indirect_subsidies) / consumption),
                    disposable_income,
                )
